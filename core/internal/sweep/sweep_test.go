...
func TestStrategiesAreNotEmpty(t *testing.T) {
	if len(Strategies()) == 0 {
		t.Fatal("no strategies")
	}
}

func TestEveryStrategyIsSocketDoable(t *testing.T) {
	hello := helloWith("gateway.discord.gg")

	for _, s := range Strategies() {
		rule, err := rules.Parse("x=" + s)
		if err != nil {
			t.Errorf("strategy %q does not parse: %v", s, err)
			continue
		}

		if _, err := plan.FromRule(hello, rule); errors.Is(err, plan.ErrNoSocketWay) {
			t.Errorf("strategy %q is not socket-doable", s)
		} else if err != nil {
